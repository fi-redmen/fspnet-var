...
from utils.plot_utils import get_energies, decoder_reconstruction, quantile_limits
from utils.misc_utils import sample, add_state_labels, order_by_state
from utils.xspec_utils import xspec_reconstruction, reduced_PG
...
def comparison_plot(
    ...
    for i, (targ, lat, axis, grey_targ) in enumerate(zip(multi_targs, multi_lats, axes.values(), grey_targs)):
        ...
        for sample_num in range(len(lat[0])):
            ...
            ms.append(m)
            m_errs.append(m_err)
            fit_lines.append(log_line(x_fit, m) if i in log_params else line(x_fit, m))
...
def rec_2d_plot(
    ...
    data_dir: 
        directory of spectra data files - if we want to do reconstruction
    '''
    title_size = 16
    label_size = 14
    tick_size = 12
    ...
    # order data by spectra
    ordered_data = order_by_state(labelled_data)
    ...
    # plot format
    fig, axes = plt.subplot_mosaic('abc', figsize=(15,5), layout='constrained')
    for i, ax in enumerate(axes.values()):
        ...
        ax.hlines(lines, energies[0], energies[-1], colors='#f2c42c', linestyles='-', linewidths=2) # adding lines to show state boundaries
        if i>0:
            ax.set_yticks([])
        ax.tick_params(axis='both', which='major', labelsize=tick_size)

    axes['a'].set_ylabel('Spectrum number', fontsize=label_size)
    axes['a'].set_title('Inputs', fontsize=title_size)
    axes['b'].set_title('Preds', fontsize=title_size)
    axes['c'].set_title('Inputs-Preds', fontsize=title_size) 
   
    for i, state in enumerate(list(dict.fromkeys(states))):   # add state labels
        axes['c'].text(10.5, lines[i] + (lines[i+1]-lines[i])/2, state)

    # color normalisation by flux and color bars
    Norm = pltcolors.LogNorm(vmin=np.min(inputs), vmax=np.max(inputs))
    resid_Norm = pltcolors.Normalize(vmin=-np.max(abs(inputs-preds)), vmax=np.max(abs(inputs-preds)))
    cbar = fig.colorbar(plt.cm.ScalarMappable(norm=Norm, cmap='Blues'), ax=(axes['a'],axes['b']), orientation='horizontal',  label='Flux', aspect=80)
    cbar_resid = fig.colorbar(plt.cm.ScalarMappable(norm=resid_Norm, cmap='seismic'), ax=(axes['c']), orientation='horizontal',  label='Flux', aspect=40)
    cbar.ax.xaxis.label.set_size(label_size)
    ...
    plot_kwargs = {'aspect':'auto', 'extent':(energies[0], energies[-1], 0, len(preds)),
                'origin':'lower', 'cmap':'Blues', 'norm':Norm, 'interpolation':'none'}
    axes['a'].imshow(inputs, **plot_kwargs)
    axes['b'].imshow(preds, **plot_kwargs)
    plot_kwargs['cmap']='seismic'
    plot_kwargs['norm'] = resid_Norm
    axes['c'].imshow(inputs-preds, **plot_kwargs)

    plt.savefig(os.path.join(dir_name,'2d_recon.png'), dpi=300)
