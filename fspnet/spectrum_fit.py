...
def gaussian_loss(predictions, target):
            ...
            return nn.GaussianNLLLoss(predictions, target[:,0] ) #(spectra)(B,240), target[:,1]**2 (B,2,240) (uncertainty))

def mse_loss(predictions, target):
    return nn.MSE(predictions, target[:,0]) #(spectra))(B,240) 


'''
 def net_init(..): 
    ...        
    #with other decoder creating code (in else)
    decoder = Decoder(...)
    net.reconstruct_loss = gaussian_loss

    #with other autoencoder creating code (in else)
    net = Autoencoder(...)
    net.reconstruct_func = gaussian_loss
    net.latent_func = mse_loss

OR net.latent_func = 0'''
#if not using latent function, relying fully on reconstruction loss
#trained decoder to expect physical loss BUT might still go out of bounds for physics
#adding latent loss function helps if you true value for some data
#why latent function=mse better than =0

#gaussian loss does MSE based on how noisy data is (prioritises less noisy data)
#might be other things better than gaussian
...
class AutoencoderNet(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor: # add ,target_uncertainty to arguments
        ...
        #return self.net[1](torch.cat(x, target_uncertainty[:,-1:]))     #ethan suggestion for variational
...
def net_init(
    ...
    if d_load_num:
        ...
        decoder.reconstruct_loss = gaussian_loss    #changes loss to gaussian loss - can look into other typs of loss
    ...
    if e_load_num:
        ...
        net.bound_loss = 0
        net.kl_loss = 0
        net.reconstruct_func = gaussian_loss    #changes autoencoder loss
    ...
    for dataset in datasets:
        ...
        #note: changed from just param_transform(dataset.params)
