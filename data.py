...
class SpectrumDataset(Dataset):
    ...
    def __init__(self, data_file: str, log_params: list[int]):
        ...
        self.spectra = self.spectra[:, :self.spectra.shape[-1] // 2 * 2]
        ...
        if 'uncertainties' in data:
            ...
            self.uncertainty = self.uncertainty[:, :self.uncertainty.shape[-1] // 2 * 2]
    ...
    def __getitem__(self, idx: int) -> tuple[ndarray, ndarray | Tensor, ndarray | Tensor]:
        ...
        #changed return statement for uncluding uncertainties in variational autoencoder
        # return self.names[idx], module.stack((
        #     self.params[idx], 
        #     self.param_uncertainty[idx])),\
        # module.stack((
        #     self.spectra[idx],
        #     self.uncertainty[idx],
        # ))
    
        # only includes uncertainties in spectra
        ...
        #when uncertainties aren't includeed 
...
def load_x_data(y_size: int) -> ndarray:
    ...
    ndarray
        x data points
    """
    x_data = np.load('../Spectrum-Machine-Learning-master/data/spectra_x_axis.npy')
